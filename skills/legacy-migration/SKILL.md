...
- **턴 종료 계약**: 구현 중의 턴은 ① 완료 보고(실제 변경+검증) 또는 ② 멈춤 조건+질문으로만 끝난다. **"진행하겠습니다"류 선언으로 턴을 끝내지 않는다** — 하겠다고 말했으면 그 턴 안에서 한다. 승인이 이미 있으면 "그래" 확인을 다시 구하지 않는다. 긴 세션에서 이 규칙이 희미해졌다 싶으면 구현 시작 전에 implement 커맨드 절차를 다시 읽는다.
- **구현 단위는 세로 슬라이스**: task 하나(= API 하나)를 domain→repository→controller→테스트까지 한 번에 관통해 완성한다. 계층별로 쪼개 중간 보고하지 않는다. 승인된 task가 여러 개면 **묶어서 연속 수행하고 보고는 끝에 한 번** — 단, 묶음 실행이 범위 확장 허가는 아니다. 멈추는 건 멈춤 조건(스펙 밖/미승인 대상/기존 코드 교체/공유 코드/미해결 실패/컨벤션 충돌)에 해당할 때뿐이다.
- **2-pass 구현**: Pass 1(동작+테스트) → Pass 2(정리: 책임 분리 / 이름 / 조건문 / null 흐름 / 중복·추상화). Pass 2 없이 완료 표시하지 않는다.
- **이름은 비즈니스 의도를 표현한다** — `data`/`result`/`value`/`flag`/`temp`/`util`/`manager`류 모호한 이름과 magic value를 쓰지 않는다. 복잡한 조건은 의미 있는 이름으로 분리하고 중첩보다 보호 절·빠른 반환을 우선한다.
- **성급한 추상화 금지** — 필요가 확인되지 않은 인터페이스/추상클래스/디자인 패턴을 미리 추가하지 않는다. 줄 수 줄이기용 의미 없는 분리·공통화도 하지 않는다.
- **요청 범위를 넘어선 리팩터링·기존 API 변경 금지.** 기존 코드와 컨벤션이 충돌하면 기존 일관성을 우선하고 질문한다.
- 주석은 코드가 무엇을 하는지 설명하지 않는다 — 이름과 구조로 표현하고, 주석은 이유·제약에만.
- **binding 컨벤션 재주입**: `docs/conventions/binding-rules.md`(Approved, 10줄 이내)를 구현 직전 다시 읽고, 구현 후 규칙별 `지켰음`/`예외(사유)` 대조표를 남긴다. 계층 책임(Controller 금지사항, domain vs UseCase 판단 기준)은 프로젝트의 `layer-responsibility` 컨벤션을 따른다.
- 외부 연동 실패를 null로 삼키지 않는다 — 원인 로그를 보존하고 명시적 실패 타입으로 표현한다.
- 테스트는 성공 경로만이 아니라 실패·경계 조건(빈 결과, 잘못된 입력, 외부 실패)도 검증한다.
