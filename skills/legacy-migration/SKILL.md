...
1. Target 프로젝트의 **승인된 컨벤션 문서**(코드 컨벤션, 아키텍처, 비즈니스 규칙)를 먼저 찾아 읽고, 스펙과 구현에 그대로 반영한다.
2. 사용자가 대화에서 컨벤션/아키텍처/비즈니스 로직을 제시하면 그것이 최우선이다.
3. 승인된 컨벤션이 없으면 반복되는 코드/테스트 패턴과 반례를 근거로 **초안만** 작성한다. 컨벤션을 발명하거나 관찰된 패턴 하나를 프로젝트 규칙으로 승격하지 않는다.
