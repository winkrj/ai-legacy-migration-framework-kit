...
> Task ID 없는 구현은 금지한다. 승인된 IMPL task를 세로 슬라이스로 구현하고, 테스트 실패와 독립 검토 발견을 승인 범위 안에서 자동 수정한다.
...
- <없음 또는 Human 승인 필요 — 승인된 수정 범위 밖 API/파일/외부 연동일 때만 멈춘다>

## 지속 실행 루프

| 시도 | 단계 | 실패 원인/발견 | 수정 | 재검증 | 결과 |
|---|---|---|---|---|---|
| 1 | 대상 테스트 / 전체 verify / 독립 검토 |  |  |  |  |
