...
description: 레거시 프로젝트의 기능을 별도의 신규 프로젝트로 옮기는 "이관(migration)" 작업에만 사용한다. 사용자가 레거시 경로를 지목하며 이관·마이그레이션·포팅을 요청하거나, 이관 스펙 승인·구현·검증을 요청할 때 호출한다. 같은 프로젝트 안에서 하는 일반 작업에는 호출하지 않는다 — 새 기능 개발, 버그 수정, 리팩터링, 코드 읽기·질문·설명, 테스트 작성, 커밋·PR 작업은 이 스킬 대상이 아니다.
---
