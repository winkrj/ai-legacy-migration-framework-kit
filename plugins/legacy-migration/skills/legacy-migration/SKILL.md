...
Codex에 서브에이전트가 없으면 아래를 **별도 스레드 또는 순차 패스**로 수행한다. 핵심은 작성한 주체가 스스로 검사하지 않는 것이다.

| 언제 | 패스 | 규칙 |
|---|---|---|
| Discover | 화면·설정에서 도달 가능한 endpoint 전수 열거 | 인용 없는 발견은 보고하지 않는다 |
| Specify 종료 | 스펙 ↔ 레거시 대조로 누락·근거약함·모순 탐지 | **새 스레드에서** 수행하고 고치지 않는다 — 지적만 |
| Discover 이후 | 개선 후보(N+1·캐싱·트랜잭션·쿼리) 탐지 | `07_Improvements.md`에만 기록, 이관 구현에 섞지 않는다 |
