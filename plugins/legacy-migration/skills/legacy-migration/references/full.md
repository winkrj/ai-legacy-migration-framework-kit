...
- **`02_Specify.md`가 단일 계약(SDD)** — `API 목록` 표는 색인이고, API마다 `### API-NNN` 상세 섹션에 계약을 쓴다: 목적 / 권한·사전조건 / 시나리오(Given/When/Then) / Request / Response(레거시 원 필드 매핑) / 레거시 호출 흐름(인용) / DB·외부 연동 / 변환 규칙 / 오류·빈 결과 / Acceptance Criteria(AC ID) / 연결 Task. GWT는 여기에만 쓴다 — OpenSpec spec.md는 포인터. 공통 규칙은 컨벤션 링크 + 케이스 예외만.
...
- **구현은 task ID 기준 + 세로 슬라이스 + 2-pass** — Implementation Permission이 Granted된 `IMPL-API-NNN`만 구현하되, task 하나는 domain→repository→controller→테스트까지 **한 번에 관통**해 완성한다(계층별 중간 보고 금지). Granted된 task가 여러 개면 **묶어서 연속 수행하고 보고는 끝에 한 번**. task ID 없는 구현은 금지 — 묶음 실행이 범위 확장 허가는 아니다(기존 코드 교체·삭제가 필요해지면 멈추고 질문). Pass 1(동작+테스트) 후 Pass 2(정리)를 별도로 돌고 04_Implement에 기록한다. Approved binding 컨벤션은 구현 직전 재주입. 구현 후 task와 validation evidence를 갱신한다.
