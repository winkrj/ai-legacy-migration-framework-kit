...
- **구현은 task ID 기준** — Implementation Permission이 Granted된 `IMPL-API-NNN`만 구현한다. task ID 없는 구현은 금지. 예상 밖 API·파일·외부 연동을 발견하면 멈추고 질문한다. 구현 후 task와 validation evidence를 갱신한다.
