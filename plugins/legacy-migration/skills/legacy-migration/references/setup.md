...
4. 루트 `AGENTS.md` 배치 — 기존 파일은 유지하고 병합 필요 여부만 안내
5. `.codex/agents/*.toml` 복사 — **Codex는 플러그인 매니페스트로 agent role을 배포할 수 없어 이 단계가 필수다**
...
- `./scripts/verify.sh --baseline`을 실행해 이관 전 빌드·테스트 결과를 `reports/baseline-verify-*.txt`에 저장한다.
