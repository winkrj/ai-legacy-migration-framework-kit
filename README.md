...
### 1.6.1 — 구현 턴 조기 종료와 hook 127 보강

승인된 구현을 시작한 뒤 `진행하겠습니다`·`다음 작업을 하겠습니다`만 출력하고 턴을 끝내는 회귀를 막기 위해 실행 지속 게이트를 추가했습니다. 부분 패치·탐색·컴파일은 종료 조건이 아니며, PASS 완료 보고 또는 근거가 있는 실제 blocker에서만 구현 턴을 끝냅니다. 사용자가 `한 번에`·`끝까지`를 명시하면 지원 환경에서 지속 goal로 이어가도록 했습니다.

Codex hook은 실행 파일 직접 호출 대신 `/bin/bash`를 명시하고 표준 PATH 및 필수 명령 검사를 추가했습니다. hook을 안전하게 실행할 수 없으면 코드 변경을 허용한 채 127로 무력화하지 않고 차단 사유를 보고합니다.
...
컨벤션을 문서에 적어두고 "읽고 지켜라"라고 해도 몇 개는 지켜지지 않았습니다. 프롬프트 층에 있는 규칙은 확률적이라, 사람이 짜든 AI가 짜든 결과가 매번 달랐습니다.

`templates/target-project/ArchitectureTest.java.template`을 추가했습니다. 기준 패키지 한 줄만 바꾸면 되고, `scripts/verify.sh`가 테스트를 돌리므로 **컨벤션 위반이 빌드 실패로** 나타납니다. `/legacy-migration:setup`이 JVM 프로젝트에서 이 템플릿을 안내합니다.

규칙을 고르는 기준을 템플릿에 명시했습니다 — **컨벤션 문서에 이미 있는 것만 옮기고, 여기서 새 규칙을 만들지 않습니다.** 승인되지 않은 규칙이 빌드를 깨면 나머지 규칙까지 신뢰를 잃습니다.

실제 프로젝트(Maven 멀티모듈)에 적용해보니 모듈 경계·계층 순서·페이징 컨벤션은 잘 지켜지고 있었고, 반대로 문서에 있는 규칙 하나는 코드가 전혀 따르지 않고 있다는 것이 드러났습니다. 지켜지지 않는 규칙은 켜지 않고 결정 대기로 남기는 편이 낫습니다.

### 1.4.0 — 프로젝트 세팅 명령과 수정 루프

세팅이 수동이었습니다. Codex 서브에이전트 role은 직접 복사해야 했고, `verify.sh`도 손으로 넣어야 했습니다. 어디에 설치됐는지 확인하는 것도 사람 몫이었습니다.

`/legacy-migration:setup`(Codex는 "이 프로젝트 세팅해줘")을 추가했습니다. 디렉터리·`verify.sh`·컨벤션 템플릿·서브에이전트 role을 한 번에 배치하고, 마지막에 실제로 동작하는지 검증합니다. 멱등해서 다시 실행해도 안전하고, 기존 파일은 덮어쓰지 않습니다. 컨벤션 내용은 생성하지 않습니다 — 사람이 정해야 binding이 되기 때문입니다.

그리고 테스트가 실패하면 멈춰서 보고만 하고 끝나던 것을, **최대 3회까지 스스로 고치고 재검증**하도록 했습니다. 3회 안에 통과하지 못하면 사람에게 넘깁니다. 착수 범위 밖 수정이 필요하거나, 실패 원인이 기존 코드에 있거나, 테스트를 약화해야 통과하는 경우는 횟수와 무관하게 즉시 멈춥니다.

Claude Code의 `Stop` 훅으로 "통과할 때까지 못 멈추게" 강제할 수도 있지만 쓰지 않았습니다. 공식 문서가 루프 보호 장치가 없다고 명시하고 있어, 고칠 수 없는 실패를 만나면 무한히 도는 사고가 납니다.

### 1.3.0 — 완료 게이트

통합 테스트를 맡겼는데 실행하지 않거나, 빌드가 깨진 채로 완료 보고가 오는 경우가 있었습니다. 원인은 완료 여부를 AI의 서술에 의존했기 때문입니다. "테스트했습니다"는 검증할 수 없습니다.

`templates/target-project/scripts/verify.sh`를 추가했습니다. 빌드 도구(gradle·maven·npm)를 자동 탐지해 빌드와 테스트를 순서대로 실행하고, 결과를 `reports/verify-<타임스탬프>.txt`에 남기고 실패하면 비정상 종료합니다. 빌드가 깨지면 테스트 단계로 넘어가지 않습니다.

구현 절차는 완료 보고 전에 이 스크립트를 실행하고 리포트 경로를 인용하도록 바꿨습니다. Validator에 `VERIFY_REPORT` 룰을 추가해, 구현이 진행된 상태인데 리포트 참조가 없으면 경고하고 FAIL 리포트를 인용한 채 완료로 표시하면 실패시킵니다.

실제 프로젝트(Maven 멀티모듈)에 적용해보니 첫 실행에서 873개 중 5개 실패가 드러났습니다. 전체를 한 번에 돌리는 수단이 없어 그동안 발견되지 않던 것이었습니다.
