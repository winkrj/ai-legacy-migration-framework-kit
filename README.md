...
> **처음이신가요?** 설치부터 첫 이관·승인·MR까지 모든 입력과 결과를 한 단계씩 보여주는
> [따라하기 가이드 (HTML)](guides/Kit-Usage-Guide.html)가 있습니다. 파일을 받아 브라우저로 열면 됩니다.
...
> ⚠️ **플러그인은 슬래시 커맨드를 만들지 않습니다.** 설치 후에는 **자연어로 요청**하면 스킬이 워크플로우를 적용합니다.
> 예: `notice-list 기능 이관을 분석 단계부터 시작해줘. 레거시 경로는 ~/work/legacy-admin`
> `/migrate-start` 같은 커맨드가 치고 싶다면 아래 방법 B를 (추가로) 설치하세요.

### Codex — 방법 B: 커스텀 프롬프트 (`/migrate-*` 커맨드)

```bash
npx --yes github:winkrj/ai-legacy-migration-framework-kit
```

`~/.codex/prompts/`에 `/migrate-start`, `/migrate-conventions` 등 커맨드 5종이 설치됩니다. A와 B는 같이 설치해도 됩니다. 상세는 [codex/INSTALL.md](codex/INSTALL.md).
...
| 하는 일 | Claude Code | Codex 플러그인 (자연어 예시) | Codex 프롬프트 (방법 B) |
|---|---|---|---|
| 컨벤션 등록 | `/legacy-migration:conventions [참고경로]` | "컨벤션을 등록해줘. 참고 프로젝트는 ~/work/team-guide" | `/migrate-conventions` |
| 이관 시작 (분석+스펙) | `/legacy-migration:start 기능명 경로` | "notice-list 기능 이관을 분석 단계부터 시작해줘. 레거시 경로는 ~/work/legacy-admin" | `/migrate-start` |
| 승인 후 구현 | `/legacy-migration:implement 기능명` | "notice-list 스펙 승인했어. 구현 진행해줘" | `/migrate-implement` |
| 문서 검사 | `/legacy-migration:validate 케이스명` | "notice-list 이관 문서를 validator로 검사해줘" | `/migrate-validate` |
| Full 모드 (고위험) | `/legacy-migration:full 기능명 경로` | "payment-refund를 Full 모드로 이관해줘" | `/migrate-full` |

Codex 플러그인 열에 슬래시가 없는 이유: Codex 플러그인은 커맨드가 아닌 **스킬**로 동작하기 때문입니다. 자연어로 요청하면 스킬이 같은 절차를 적용합니다.
