---
description: 이관 대상 프로젝트에 kit 실행 환경 설치 (AGENTS.md·디렉터리·verify.sh·컨벤션·서브에이전트)
...
4. 루트 `AGENTS.md` 배치 (기존 파일은 유지)
5. Codex 사용 시 `.codex/agents/*.toml` 복사 (Claude Code는 플러그인이 직접 제공하므로 불필요)
6. 설치가 실제로 동작하는지 검증하고 요약 출력

기존 파일은 덮어쓰지 않는다. 이미 있으면 그대로 둔다.

[실행 후 사용자에게 안내할 것]
- 스크립트 출력의 `✗` 항목이 있으면 그것부터 해결해야 한다. 특히 빌드 도구 미탐지는 `scripts/verify.sh`의 탐지 블록을 직접 채워야 한다.
- `./scripts/verify.sh --baseline`을 실행해 **이관 전 빌드·테스트 기준선**을 저장하도록 안내한다. 이미 실패하고 있으면 이관 결과와 섞여 원인을 가릴 수 없다.
