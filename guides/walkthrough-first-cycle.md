...
> **더 빠른 방법:** 아래 2~5단계는 [`prompts/start-migration.md`](../prompts/start-migration.md) 프롬프트 하나로 자동화된다 — 빈칸 2개 채워 붙여넣으면 AI가 세팅→분석→스펙 초안까지 만들고 승인 지점에서 멈춘다. 이 문서는 각 단계에서 무슨 일이 일어나는지 이해하고 싶거나, 수동으로 통제하고 싶을 때 보는 상세 버전이다.
